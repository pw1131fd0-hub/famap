...
export function generateICalendar(trip: TripData): string {
  ...
  const locationList = locations
    .slice(0, 5)
    .map(loc => {
      const locName = typeof loc.name === 'string' ? loc.name : loc.name.en;
      return `- ${locName}`;
    })
    .join('\\n');
...
LOCATION:${locations.slice(0, 2).map(l => {
  const addr = typeof l.address === 'string' ? l.address : l.address?.en;
...
export function generateTripShareData(trip: TripData): string {
  ...
  // Use encodeURIComponent for proper UTF-8 encoding of JSON
  ...
  return `
    ...
    <body>
      <div class="header">
        ...
        <p>${t('date')}: ${trip.date} (${new Date(trip.date).toLocaleDateString()})</p>
      </div>
      ...
      <div class="section">
        ...
        ${locations.slice(0, 10).map(loc => {
          const locName = typeof loc.name === 'string' ? loc.name : loc.name[language];
          const locAddress = typeof loc.address === 'string' ? loc.address : loc.address?.[language];
          return `
            <div class="location-item">
              <h3>${locName}</h3>
              ${loc.category ? `<p><strong>${t('category')}</strong>: ${loc.category}</p>` : ''}
              ${locAddress ? `<p><strong>${t('address')}</strong>: ${locAddress}</p>` : ''}
            </div>
          `;
        }).join('')}
      </div>
  ...
  if (locations.length === 0) {
    ...
    locations.forEach((loc, idx) => {
      rows.push([
        ...
        idx === 0 ? members : '',
        idx === 0 ? trip.notes : '',
        typeof loc.name === 'string' ? loc.name : loc.name[langKey],
        typeof loc.address === 'string' ? (loc.address || '') : (loc.address?.[langKey] || ''),
        loc.category || ''
      ]);
    });
  }
